...
import Unit.Effects.FileSystem.IO qualified
import Unit.Effects.FileSystem.OsPath qualified
...
main =
  defaultMain $
    withResource setup teardown $ \args ->
      testGroup
        "Unit Tests"
        [ Unit.Effects.FileSystem.IO.tests args,
          Unit.Effects.FileSystem.OsPath.tests
        ]

setup :: IO OsPath
setup = do
  tmpDir <-
    (\s -> s </> [osp|effects-fs-utils|] </> [osp|unit|])
