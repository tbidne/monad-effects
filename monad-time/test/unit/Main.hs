...
-- Property tests:
-- - withTiming
-- - from/ToDouble round trip ish
--
-- CallStack goldens will prob fail
...
timesAction = testCase "Times an action" $ do
  ...
  assertBool (show ts <> " >= 0.5 s") $ ts >= MkTimeSpec 0 500_000_000
  assertBool (show ts <> " <= 1.5 s") $ ts <= MkTimeSpec 1 500_000_000
